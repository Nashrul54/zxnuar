...
var footer_nya =`Zxnuxr-Botz @${setting.ownerNumber.split("@")[0]}`
let menu_nya = `_*═══「 Zxnuxr-Botz 」═══*_
...
{buttonId: '#groupbot', buttonText: {displayText: '️「 GROUPBOT 」'}, type: 1}
]
...
var user_name = `#Beban${diacuk}`
let object_user = {"id": sender, "name": user_name, "seri": res_us, "premium": false}
db_user.push(object_user)
fs.writeFileSync('./database/pengguna.json', JSON.stringify(db_user))
mentions(`Memuat Beban dunia... @${sender.split("@")[0]}`, [sender])
await sleep(1500)
var verify_teks =`_*───「 TERVERIFIKASI 」────*_
...
//------------ costum menu Zxnuxr-Botz ----------//
case 'rules':{
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
reply(`
_*══「 DIWOCO RULES E 」══*_

ora oleh spam op maneh telpon 
mlh ke blokir engko koar² kon
cokkkk!
 
Sekian rules teko bot kne lek onk
seng kurang apik yo slh mu
...
_*══「 MONGGO DONASI 」══*_
 ...
 ¤ Pulsa : 083177950713/083107995316
 ¤ Dana : lali no dana kne
 ¤ Gopay : arep gawe sek rek
...
None:
room chat ini bersifat anonim
jika kamu tidak tau partnermu
kecuali dia menggunakan nama aslin
atau memberitahukan informasi darinya.
...
let btn_room = [{ buttonId: `${prefix}auto_room ${sender}|${penerimanyo}|${roomC}`, buttonText: { displayText: '⋮☰ TERIMA' }, type: 1 }]
var but_room = {
...
let btn_menfes = [{ buttonId: `${prefix}auto_room ${sender}|${num}@s.whatsapp.net|${roomC}`, buttonText: { displayText: '⋮☰TERIMA' }, type: 1 }]
var button_menfess = {
text: text_menfess,
...
let btn_menfes = [
{ buttonId: `${prefix}auto_room ${sender}|${num}@s.whatsapp.net|${roomC}`, buttonText: { displayText: '「 TERIMA 」' }, type: 1 },
{ buttonId: `${prefix}tolak_secret ${sender}`, buttonText: { displayText: '「 TOLAK 」' }, type: 1 }
]
var button_menfess = {
text: text_menfess,
footer: 'Klik button untuk membalas chat.',
...
let btn_room = [
{ buttonId: `${prefix}auto_room ${penerimanyo}|${penerimanya}|${roomC}`, buttonText: { displayText: '「 TERIMA 」' }, type: 1 },
{ buttonId: `${prefix}tolak_secret ${penerimanyo}`, buttonText: { displayText: '「 TOLAK 」' }, type: 1 }
]
var but_room = {
text: text_tersambung,
footer: 'Klik button untuk menerima chat.',
...
mentions(`@${sender.split('@')[0]}\nTerdeteksi Mengirim Gambar\nAuto Convert Ke Sticker`, [sender])
